...
from database import get_debt_board, get_ping_by_id
...
def debt_card(row: dict[str, Any]) -> str:
    lines = [
...
async def _show_board(click: Click, code: str, page: int) -> None:
    board, window, has_more = await load(code, page)
    selected = marks(click.sender_id)
    rows = segment_rows(board.get("rows") or [], code)
    await safe_edit(click.event,
                    board_card(board, rows, code, len(selected)),
                    buttons=board_keyboard(window, code, page, has_more, selected),
                    link_preview=False)
...
async def _show_card(click: Click, ping_id: int, code: str, page: int) -> None:
    ...
    if not row:
        ...
        return
    await safe_edit(click.event, debt_card(row),
...
async def handle(click: Click) -> None:
    ...
    if action in ("c", "x"):
        ...
        await _show_board(click, code, page)
        return
    await _show_board(click, "a", 1)
