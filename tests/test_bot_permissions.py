...
class BroadcastFilterTests(unittest.TestCase):
    ...
    def test_account_scoped_key_only_gets_its_own_wins(self):
        ...
        self.assertEqual(filter_broadcast_members([member], "win", set(), ["@muver"]), [member])
        self.assertEqual(filter_broadcast_members([member], "win", set(), ["@other"]), [])
    ...
    def test_legacy_member_without_grants_still_receives(self):
        self.assertEqual(len(filter_broadcast_members([self._member(1)], "win", set(), ["@any"])), 1)
